...
void Recognize::removeRecognizedPoints(unsigned char (&image)[100][1000])
{
    ...
    for (int i =0 ;i<queueSize;i++) {
        ...
        case 3:
            ...
            break;
        case 4:
            image[pointQueue.getFront().y][pointQueue.getFront().x] = 6;
            break;
        case 5:
            image[pointQueue.getFront().y][pointQueue.getFront().x] = 7;
            break;
        }
...
Stroke Recognize::findStroke(unsigned char  (&imageBuffer)[100][1000])
{
            ...
            && imageBuffer[point.y][point.x+3] == 1 && imageBuffer[point.y][point.x+4] == 1)
    {
        ...
        if(n>15)
        {
            ...
            strokeTemp.scale = n;
            return(strokeTemp);
        }else//否则该笔画为垂直型笔画或者斜线型笔画或者为一撇或者一捺
        {
            ROS_INFO("3\n");
            //左侧有点表示该笔画为竖线
            ...
            }else//否则为竖弯钩或者竖弯
            {
                ...
                if(imageBuffer[point.y+m+1][point.x+n-1] == 1)
                {
                    ...
                    {
                        //ROS_INFO("layerNum1:%d layerNum2:%d",layerNum1,layerNum2);
                        //水平遍历连通的点
                        ...
                        for (;neighbourNum1>=0 || neighbourNum2>=0 ;) {
                            ...
                            if(layerNum2>=0 && neighbourNum2 >=0 && imageBuffer[point.y+layerNum2][start_x2-neighbourNum2]==1){
                                ...
                                //ROS_INFO("end_x2 = %d neighbourNum2 = %d",end_x2,neighbourNum2);
                                //将识别过的点存入点队列
                        ...
                        //左右起始点一层一层下移
                        int count=0;
                        for (count=0;imageBuffer[point.y+layerNum1+1][start_x1-count] ==  0 && layerNum1>=0;) {
                            if(count<25 && imageBuffer[point.y+layerNum1+1][start_x1-count] ==  0 && layerNum1>=0){
                                count++;
                                //ROS_INFO("count = %d ",count);
                            }
                            else if (layerNum1>=0){//大于25则层数已遍历完
                        ...
                        for (count=0;imageBuffer[point.y+layerNum2+1][start_x2-count] ==  0 && layerNum2>=0;) {
                            if(count<25 && imageBuffer[point.y+layerNum2+1][start_x2-count] ==  0 && layerNum2>=0){
                                count++;
                                //ROS_INFO("count = %d ",count);
                            }
                            else if (layerNum2>=0){//大于25则层数已遍历完
                    ...
                    ROS_INFO("d");
                    ROS_INFO("x1:%d x2:%d y1:%d y2:%d",end_x1,end_x2,layerNumMin,layerNumMax);
                    for (int i=end_x1;i<=end_x2;i++) {
                        for (int j = point.y+layerNumMin-1;j<=point.y+layerNumMax-1;j++) {
                            //将识别过的点存入点队列
                            pointTemp.x = i;
                            pointTemp.y = j;
                            pointQueue.push(pointTemp);
    ...
    for ( int j = 0; j < 100; j++ ){
        for ( int i = 0; i < 1000; i++ ){
            if(image[j][i] == 1 && pointQueue.isEmpty()){
                ...
                //ROS_INFO("z");
                //Travel the stroke queue
                strokeQueue.QueueTravers();
            }
        }
    }
}
